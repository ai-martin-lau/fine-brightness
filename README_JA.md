<p align="center">
  <a href="README.md">English</a> · <a href="README_ZH.md">简体中文</a> · <a href="README_JA.md">日本語</a> · <a href="README_KO.md">한국어</a> · <a href="README_ES.md">Español</a>
</p>

# Fine Brightness（明るさ微調整）

1% 刻みでの明るさ調整に対応した、macOS 向けの明るさ微調整ツールです。

## ダウンロード

最新バージョンは [Releases ページ](https://github.com/ai-martin-lau/fine-brightness/releases/latest) からダウンロードできます。

直接ダウンロード: [`fine-brightness.dmg`](https://github.com/ai-martin-lau/fine-brightness/releases/latest/download/fine-brightness.dmg)

## インストール

1. DMG を開きます。
2. `Fine Brightness.app` を Applications にドラッグします。

初回起動時に、開発元を確認できないと macOS に表示された場合は、App を右クリックして「開く」を選択してください。

## スター推移

[![スター推移グラフ](https://api.star-history.com/svg?repos=ai-martin-lau/fine-brightness&type=Date)](https://star-history.com/#ai-martin-lau/fine-brightness&Date)
